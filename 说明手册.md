...
![Version](https://img.shields.io/badge/版本-1.5.4-important) 
![Clones](https://img.shields.io/badge/克隆次数-465-Red)  

## 动态

2020/08/1 7.3-8.31本包通过克隆方式下载的次数达到465次。
...
2020/08/23 v1.5.0 1、增加新功能loh，用于计算VOC时间序列的OH反应活性，输入和输出格式类似ofp功能。2、ofp和loh增加是否显示警告信息的选项。（某个VOC大类如果没有值，默认会有警告信息）。3、更新了计算最大八小时臭氧的功能dm8n。可设置数据有效性要求，默认基于国标的有效性规定（每8小时中至少有6个小时平均浓度值、当天至少有14个有效8小时平均浓度值）。
...
批量计算、分析线性回归。x和y以及填充颜色（z）三个维度都可以选择多个物种。支持第四维度操作即数据分组（t）。详情请点击阅读：[批量线性回归分析利器anylm——FOQAT包的重要更新](https://zhuanlan.zhihu.com/p/162841404)。 
...
取决于函数中的参数`outputmode`。填1只输出1个表：最大八小时臭氧。填2输出1个列表，其中包含4个表：八小时臭氧、每8小时中小时平均浓度值的个数统计、 每天有效8小时平均浓度值的个数统计、最大八小时臭氧。
...
7. Con_ppbv_group：VOC按大类分的体积浓度时间序列；
8. Con_ppbv_group_mean：VOC按大类分的体积浓度均值及占比（由大到小排序）； 
9. KOH_Result：kOH值匹配结果；
10. LOH_Result：VOC按物种分的LOH时间序列；  
11. LOH_Result_mean：VOC按物种分的LOH均值及占比（由大到小排序）；  
12. LOH_Result_group：VOC按大类分的LOH时间序列；
13. LOH_Result_group_mean：VOC按大类分的LOH均值及占比（由大到小排序）。
...
群组二维码：  
<img src="https://s1.ax1x.com/2020/09/03/w9yzdO.jpg" align="center" width="180" />   

作者二维码：    
<img src="https://s1.ax1x.com/2020/09/03/w9yxeK.jpg" align="center" width="180" />   
