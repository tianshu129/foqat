...
![Clones](https://img.shields.io/badge/克隆次数-405-Red)  

## 动态

2020/08/16 7.3-8.16本包通过克隆方式下载的次数达到405次。

2020/08/14 v1.5.2 更新了ofp和loh的帮助文档。

2020/08/14 v1.5.1 修正ofp和loh的单位问题。输入的数据单位如果为微克每立方米，则计算时将先转化为标况（25度、101.325千帕）下的数据，后计算。

2020/08/14 v1.5.0 1、增加新功能loh，用于计算VOC时间序列的OH反应活性，输入和输出格式类似ofp功能。2、更新了计算最大八小时臭氧的功能dm8n。可设置数据有效性要求，默认基于国标的有效性规定（每8小时中至少有6个小时平均浓度值、当天至少有14个有效8小时平均浓度值）。
