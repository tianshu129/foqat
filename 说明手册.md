...
2021/07/21 V1.6.6.1 修复transp绘图函数引用问题。

2021/07/21 V1.6.6 修复avri只计算一个物种会报错的情况。修复trs无法计算秒分辨率的情况。新增afp计算VOC的气溶胶生成潜势。新增transp转换气溶胶粒径数据格式。新增trsp重采样气溶胶粒径数据。新增avrip计算气溶胶粒径数据的平均变化。
...
* [计算气溶胶生成潜势（AFP）](#计算气溶胶生成潜势afp-afp) 
...
* [转换粒径数据格式](#转换粒径数据格式-transp)  

* [粒径数据重采样](#粒径数据重采样-trsp)  

* [粒径数据平均变化](#粒径数据平均变化-avrip)  
...
计算VOC时间序列的“气溶胶生成潜势”（AFP）。  AFP输出单位为微克每立方米（ug/m3）。
匹配与VOC物种名对应的CAS号，通过CAS号匹配“最大增量反应活性”（MIR）值，并用于时间序列计算。  
注：缺失值在计算（例如求和）时将自动忽略。  
注：大类分类中，生物源VOC（BVOC）单独于烯烃之外列出。生物源物种包括：异戊二烯、α-蒎烯、β-蒎烯。  
气溶胶平均产率值来自“W. Wu, B. Zhao, S. Wang, J. Hao, Ozone and secondary organic aerosol formation potential from anthropogenic volatile organic compounds emissions in China. J Environ Sci. 53, 224–237 (2017)”。  
注：如果输入VOC物种包含m,p-Xylene，会自动平均拆分为m-Xylene和p-Xylene。

* #### 用法

``` r
afp(df, inunit = "ppbv", t = 25, p = 101.325, stcd=FALSE, sortd =TRUE, chn=FALSE)
```
...
输出1个列表，其中包含5个表：
1. SOAY_Result：SOA平均产率值匹配结果；
2. AFP_Result：VOC按物种分的AFP时间序列；  
3. AFP_Result_mean：VOC按物种分的AFP均值及占比（由大到小排序）；  
4. AFP_Result_group：VOC按大类分的AFP时间序列；
5. AFP_Result_group_mean：VOC按大类分的AFP均值及占比（由大到小排序）。
...
转换粒径数据格式。互相转换三列式（第一列为时间，第二列为各粒径段中间粒径，第三列为数浓度）和多列式（第一列时间，后面每列为各个粒径段数浓度）。

* #### 用法
``` r
trsp(df, prplot=TRUE, logy=TRUE, ybk=c(10,100,500,1000), nlmt=20000, colsz=10)
```
* #### 参数

| 变量名             | 含义                              | 默认值                     | 示例值/备注                           |
| ------------------| ----------------------------------|----------------------------|--------------------------------------|
| `df`              | 粒径数据时间序列的数据框。三列式（第一列为时间，第二列为各粒径段中间粒径，第三列为数浓度）和多列式（第一列时间，后面每列为各个粒径段数浓度）                   |                            |                                     |
| `prplot`            | 决定是否打印粒径谱图                |TRUE                            |不打印可以节约计算时间|
| `logy`            | 决定y轴是否采用对数坐标             |TRUE                            ||
| `ybk`           | y轴的间隔（对数坐标轴）                       | c(10,100,500,1000)                           |                                      |
| `nlmt`              | 数浓度颜色比例尺上限                    | 20000                       | |
| `colsz`              | 粒径谱图中每个粒径段柱子宽度                    | 10                      |             |              

* #### 输出

输出一个数据框，三列式（第一列为时间，第二列为各粒径段中间粒径，第三列为数浓度）或多列式（第一列时间，后面每列为各个粒径段数浓度）。并打印图。  
...
* #### 用法
``` r
trsp(df, bkip, st=NULL, et=NULL, na.rm = TRUE, cpms=TRUE, ybk=c(10,100,500,1000), nlmt=20000, colsz=10)
```
* #### 参数

| 变量名             | 含义                              | 默认值                     | 示例值/备注                           |
| ------------------| ----------------------------------|----------------------------|--------------------------------------|
| `df`              | 粒径数据时间序列的数据框。三列式（第一列为时间，第二列为各粒径段中间粒径，第三列为数浓度）                   |                            |                                     |.
| `bkip`            | 重采样的时间区间长度                |                            | '1 hour' 。注意：时间分辨率的单位需要与开始时间的最小非零位的单位相对应（或者比它小），例如设置开始时间是“2020-06-09 00:02:00”，则最小非零位是分钟。如果你要换算成小时分辨率，时间分辨率只能写”60 mins"不能写“1 hour"。如果你想要得到整点间隔的结果，则需要设置开始时间st="2020-06-09 00:00:00”。|
| `st`              | 重采样的开始时间                    | NULL                       | 默认取时间序列的第一个值。开始时间决定了新时间序列的第一个时间。例如原始序列开始时间为“2020-06-09 00:02:00”，则新时间序列也会从“2020-06-09 00:02:00”开始。注意如果要换算成小时分辨率的话，此处时间分辨率必须是写“60 mins"。因为你的开始时间并不是整点，所以不可以写”1 hour"(见bkip参数的备注）。如果你想要得到整点间隔的结果，则需要设置开始时间st="2020-06-09 00:00:00”。|
| `et`              | 重采样的结束时间                    | NULL                      | 默认取时间序列结束时间。               |              
| `na.rm`           | 是否排除NA值                       | TRUE                       |                                      |
...
输出一个数据框和粒径谱图。数据框内容为新时间分辨率的粒径数据时间序列。  
...
* #### 用法
``` r
avrip(df, bkip, mode = "recipes", value = "day", st = NULL, et = NULL, na.rm = TRUE, digits = 2, ybk=c(10,100,500,1000), nlmt=20000, colsz=10)
```
* #### 参数
...
输出为一个列表（包含两个三列式数据表：平均变化、方差）和粒径谱图。每个表第一列为周期内的序号，第二列为中间粒径，第三列为数浓度。需要注意的是，当模式用"ncycle"或"custom"时，起始时间决定了平均变化的结果中第一个要素的开始时间。例如：数据第一个时间为"2010-05-01 12:00:00"， 分辨率为1小时，模式用"ncycle"，”value"用24，则平均变化结果的是从12点开始的日均变化（24个数）。   
...
2021/05/04 v1.6.5 测试上传zenodo（CRAN1.6.5取消示例）。
...
父母、妻子、山东大学薛丽坤教授、中国环境科学研究院李红研究员、中国环境科学研究院李云凤、中国科学院重庆绿色智能技术研究院陈木兰、中国环境科学研究院张浩、中国环境科学研究院张玉洁、中国环境科学研究院孙洁亚、北京大学李佳荫、中国科学院安徽光学精密机械研究所张国贤、山东大学张英南、山东大学牟江山、香港城市大学郑棚罡、Environmental Defense Fund的Rishabhshah博士。
