...
## 请立即通过Github升级至v1.7.1

## 动态

2021/08/17 V1.7.1   
avri：修复avri非“custom”模式trs采样问题，新设置为非"custom"模式下才trs重采样。"custom"模式下把指定列移动到第一列。修复风速列名为“ws”时，处理风数据报错的问题。  
trs：取消st和et时间格式限制。自动移除时间列中的NA对应的行。修复风速列名为“ws”时，处理风数据报错的问题。  
vocct、loh、ofp：移除colid参数。采用新匹配方法。间对二甲苯不再自动拆分。ofp函数中增加中国MIR值选项。  
transp：新增函数，用于转换粒径数据格式。  
nsvp：新增函数，用于计算粒径数据的相关参数。  
fm：用于设定图的基本风格。  
...
``` r
ofp(df, inunit = "ppbv", outuint = "ppbv", t = 25, p = 101.325, stcd=FALSE, sortd =TRUE, colid = 1, chn=FALSE， mtype="usa")
```
...
| `sortd`           | 逻辑值，是否为VOC时间序列中的物种排序。         | TRUE                       |排序优先级一次为：VOC大类、分子质量、MIR值。 VOC大类包括c("Alkanes", "Alkenes", "BVOC", "Alkynes", "Aromatic_Hydrocarbons", "Oxygenated_Organics", "Other_Organic_Compounds", "Unknown") 。其中"Alkenes"不包含生物源"BVOC"。|
| `chn`           | VOC物种名是否为中文？                   |FALSE                       |                                      |
| `mtype`           | 使用美国版本MIR值还是中国版本？                   |"usa"                       |"usa"或"chn"                                      |
