...
## 动态
注意！！！为便于接收包的更新通知或者交流，请添加[我的微信](https://github.com/tianshu129/foqat/blob/master/img/wechat.jpg) ，以进入FOQAT用户微信群组。或加入 [FOQAT用户邮件群组](https://groups.google.com/d/forum/foqat)，一个群组邮件列表和Q/A论坛。  
...
v1.1.0全面更新了R包中的帮助文档。增加英文版说明手册。取消了数据重采样和平均变化两个函数的输出结果中风速、风向列自动移动到第2、3列的设定，风向、风速将保持原始列号。  

``` r
#注意：v1.1.0为重要更新。请覆盖升级至当前新版本。升级后请重启Rstudio方能生效。
...
批量计算线性回归。x和y以及填充颜色（z）三个维度都可以选择多个物种。支持第四维度操作即数据分组（t）。
...
| `range.y`         | 时间序列的数据框                    | "interval"                  |                                      |
| `range.x`         | 时间序列的数据框                    | "interval"                  |                                      |
| `nperm`           | 时间序列的数据框                    | "interval"                  |                                      |
| `scint`           | 时间序列的数据框                    | "FALSE"                     |                                      |
| `dign`            | 时间序列的数据框                    | 1                           |                                      |
| `zfill`           | 时间序列的数据框                    | "lightgray"                 |                                      |
| `ppsize`          | 时间序列的数据框                    | 2                           |                                      |
| `showinfo`        | 时间序列的数据框                    | TRUE                        |                                      |
| `ptsize`          | 时间序列的数据框                    | 12                          |                                      |
| `pncol`           | 汇总页面中，图的列数                 | NULL                       |                                      |
...
#批量进行线性回归分析
x=anycor(df, xd=c(2,3), yd=6, zd=4, td=7,dign=3)
View(x)
```
