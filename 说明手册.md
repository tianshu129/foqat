...
输出一个数据框，三列式（第一列为时间，第二列为各粒径段中间粒径，第三列为数浓度）或多列式（第一列时间，后面每列为各个粒径段数浓度）。  
...
输出为一个列表（包含两个三列式数据表：平均变化、方差）。每个表第一列为周期内的序号，第二列为中间粒径，第三列为数浓度。需要注意的是，当模式用"ncycle"或"custom"时，起始时间决定了平均变化的结果中第一个要素的开始时间。例如：数据第一个时间为"2010-05-01 12:00:00"， 分辨率为1小时，模式用"ncycle"，”value"用24，则平均变化结果的是从12点开始的日均变化（24个数）。   
