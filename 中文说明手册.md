...
## R语言程序的安装
请先安装R语言原生程序，后安装R语言集成开发平台Rstudio (Desktop版本即可）。然后打开Rstudio即可在控制台开始操作。请点击链接：
...
函数由“函数名（参数1，参数2=参数2的默认值，···）”构成。不同参数之间用逗号隔开。函数中的符号都是英文符号。可将运行结果赋值给任意变量。例如以下代码将生成一个包含3个1的向量集：
``` r
x=rep(1,3)
View(x)
```
...
如果默认值符合你的需求，则无需列出该参数。例如，以下两句函数得到的结果是一样的（statdf参数中，n有默认值2，你要设置的n也是2）：  
``` r
statdf(x, n = 2)
statdf(x)
```
...
## 本包的安装
本包当前不是官方包，需要安装开发工具包"devtools"。然后用"devtools"中的install_github函数在线安装本包。
``` r
# Install from GitHub:
# install.packages("devtools")
devtools::install_github("tianshu129/foqat")
```
如果运行'install.packages("devtools")'报错，请改变CRAN仓库服务器至附近的服务器。    
Rstudio中：Tools->Global options->Packages->Primary CRAN repository  

如果运行'devtools::install_github("tianshu129/foqat")'报错，请在IE中启用TLS 1.2。  
详见：https://github.com/r-lib/remotes/issues/130  
